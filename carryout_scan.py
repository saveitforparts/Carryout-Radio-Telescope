...
el_start = int(input('Starting Elevation in degrees (22-73, default 22): ') or 22)
if el_start < 22:
...
el_end = int(input('Ending Elevation in degrees (default 73): ') or 73)
if el_end < 22:
	print('Elevation out of range, setting to 22')
	el_end=21
if el_end > 73:
...
sdr.center_freq = user_freq*1e6 # - 200e3 #MHz, with offset to avoid DC spike CHECK THIS
sdr.gain = user_gain
...
command = ('g ' + str(az_start) + ' ' + str(el_start) + '\r').encode('ascii')
...
for elevation in range (el_start,el_end+1):
		...
		for azimuth in range (az_start,az_end):
			...
			command = ('g ' + str(azimuth) + ' ' + str(elevation) + '\r').encode('ascii')
